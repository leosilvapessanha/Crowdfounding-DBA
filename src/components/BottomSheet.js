...
import { escapeHtml, formatMoney, icon } from './utils.js';
 ...
 *
 * @param {{ rewards: Array, campaign: object, ended?: boolean }} opts
 */
export function BottomSheet({ rewards, campaign, ended = false }) {
        ...
        <div class="pt-4 pb-2">
          ...
          <div class="relative mb-3">
            ...
            <input
              ...
              placeholder="0,00"
              ${ended ? 'disabled' : ''}
              class="w-full pl-11 pr-4 py-3.5 rounded-xl border-2 border-slate-200 bg-slate-50/50 focus:border-blue-500 focus:bg-white focus:ring-4 focus:ring-blue-50 outline-none transition-all text-[20px] font-outfit font-bold text-slate-900 placeholder-slate-300 disabled:opacity-60 disabled:cursor-not-allowed"
            >
          </div>
          <p id="bs-donation-amount-error" class="hidden text-[12px] text-red-500 font-inter -mt-2 mb-3"></p>

          <!-- Quick-pick amounts -->
          <div class="flex gap-2 mb-5">
            <button type="button" class="bs-donation-quick flex-1 py-2.5 rounded-lg text-[13px] font-bold text-slate-600 bg-slate-100 hover:bg-blue-50 hover:text-blue-600 border border-transparent hover:border-blue-200 transition-all disabled:opacity-60 disabled:cursor-not-allowed" data-amount="25" ${ended ? 'disabled' : ''}>R$ 25</button>
            <button type="button" class="bs-donation-quick flex-1 py-2.5 rounded-lg text-[13px] font-bold text-slate-600 bg-slate-100 hover:bg-blue-50 hover:text-blue-600 border border-transparent hover:border-blue-200 transition-all disabled:opacity-60 disabled:cursor-not-allowed" data-amount="50" ${ended ? 'disabled' : ''}>R$ 50</button>
            <button type="button" class="bs-donation-quick flex-1 py-2.5 rounded-lg text-[13px] font-bold text-slate-600 bg-slate-100 hover:bg-blue-50 hover:text-blue-600 border border-transparent hover:border-blue-200 transition-all disabled:opacity-60 disabled:cursor-not-allowed" data-amount="100" ${ended ? 'disabled' : ''}>R$ 100</button>
            <button type="button" class="bs-donation-quick flex-1 py-2.5 rounded-lg text-[13px] font-bold text-slate-600 bg-slate-100 hover:bg-blue-50 hover:text-blue-600 border border-transparent hover:border-blue-200 transition-all disabled:opacity-60 disabled:cursor-not-allowed" data-amount="250" ${ended ? 'disabled' : ''}>R$ 250</button>
          </div>

          <button type="button" id="bs-donation-submit" data-project-id="${escapeHtml(campaign.id)}" ${ended ? 'disabled' : ''} class="w-full bg-gradient-to-r from-blue-600 to-blue-500 hover:from-blue-700 hover:to-blue-600 text-white font-bold py-3.5 rounded-xl transition-all shadow-lg shadow-blue-600/20 flex items-center justify-center gap-2 text-[15px] disabled:opacity-50 disabled:cursor-not-allowed disabled:from-slate-400 disabled:to-slate-400">
            ${ended ? 'Campanha encerrada' : `Apoiar este projeto ${icon('arrow-right', 'w-5 h-5')}`}
          </button>
          <p class="text-center text-[11px] text-slate-400 font-inter mt-2">${ended ? 'O prazo para apoiar esta campanha já terminou.' : 'O criador recebe por etapa. A cada checkpoint, você pode pedir o resto de volta.'}</p>
        </div>
  ...
  if (bsInput && bsQuickBtns.length) {
    bsQuickBtns.forEach((btn) => {
      btn.addEventListener('click', () => {
        // Mesmo motivo do painel lateral: a máscara conta centavos, então o atalho escreve
        // o valor já formatado e deixa o handler da máscara normalizar.
        bsInput.value = formatMoney(Number(btn.dataset.amount));
